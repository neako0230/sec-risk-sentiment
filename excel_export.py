...
def main():
    ...
    for p in sorted(DATA.glob("*_analysis_correlations.csv")):
        ticker = p.name.split("_")[0].upper()
        try:
            dfc = pd.read_csv(p); dfc["ticker"] = ticker
    ...
    # 5) 封面
    cover = pd.DataFrame({
        "key": ["generated_at","project_dir","data_dir","plots_dir","tickers_detected","files_written"],
        "value": [
            timestamp, str(ROOT), str(DATA), str(ROOT/"plots"),
            ", ".join(sorted({p.name.split("_")[0].upper() for p in DATA.glob("*_regressions_sig_p05.csv")})) or "(none)",
            "signal_shortlist, signal_summary, *_sig, correlations_all, events_all, events_summary"
        ]
    })
